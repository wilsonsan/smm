...
import { useMemo, useRef, useState, type ReactNode, type SVGProps } from "react";
...
function parseInitialEditorState(mediaAsset: MediaAssetEditorProps["mediaAsset"]): EditorSnapshot {
  ...
  const initialAspectKey = typeof history?.aspectRatio === "string"
...
export function MediaAssetEditor({ mediaAsset }: MediaAssetEditorProps) {
  ...
  const originalImageUrl = useMemo(
    () => (originalVariantId ? getMediaVariantUrl(originalVariantId) : null),
    [originalVariantId],
  );
  ...
  const currentAspectRatio = useMemo(
    () => resolveAspectRatio(aspectKey, originalAspectRatio),
    [aspectKey, originalAspectRatio],
  );
        ...
        <aside className="media-editor-sidebar">
          {activeTool === "crop" ? (
            ...
            <div className="media-editor-panel">
              ...
              <p className="helper-text">Annotation tools are staged for the next phase. The page is ready for text, arrows, and highlights once that layer is added.</p>
            </div>
          )}
        </aside>
      </div>
